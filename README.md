...
| `UPDATE_IF_IDLE` | `true` | Only update when no players are connected. The count comes from the server log; if the log cannot be read while the server is up, the update is skipped rather than risked |
...
| `BACKUPS_IF_IDLE` | `false` | Only back up when no players are connected, counted the same way as `UPDATE_IF_IDLE` |
